...
public class EffectEntry extends ShopEntry{

    public static Codec<EffectEntry> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            Registries.ITEM.getCodec().fieldOf("icon").forGetter(EffectEntry::getIcon),
            Registries.STATUS_EFFECT.getCodec().fieldOf("effect").forGetter(EffectEntry::getEffect),
            Cost.CODEC.listOf().fieldOf("costs").forGetter(EffectEntry::getAvailableTiers)
    ...
    EffectEntry(Item icon, StatusEffect effect, List<Cost> availableTiers) {
        this.icon = icon;
        this.effect = effect;
